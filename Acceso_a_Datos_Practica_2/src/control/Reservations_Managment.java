...
public class Reservations_Managment {
	...
	public boolean removeReservation(Reservation reservation) {
		...
		if (!list.isEmpty()) {
			try {
				session.beginTransaction();
				Reservation removedReservation = (Reservation) session.get(Reservation.class,
	...
	public boolean modifyReservation(Reservation reservation) {
		...
		if (!list.isEmpty()) {
			try {
				session.beginTransaction();
				Reservation updatedReservation = (Reservation) session.get(Reservation.class,
						reservation.getIdReservation());
	...
	/*
	 * public ArrayList<String> reservedDates() { ArrayList<String> reservedDates =
	 * new ArrayList<String>(); Query query =
	 * session.createQuery("from Reservation"); ArrayList<Reservation> result =
	 * (ArrayList<Reservation>) query.list(); DateTimeFormatter formatter =
	 * DateTimeFormatter.ofPattern("dd-MM-yyyy", Locale.getDefault()); if
	 * (!result.isEmpty()) { for(int x=0; x<result.size(); ++x) { LocalDate start =
	 * LocalDate.parse(result.get(x).getStartDate(), formatter); LocalDate end =
	 * LocalDate.parse(result.get(x).getEndDate(), formatter); ArrayList<LocalDate>
	 * dates = new ArrayList<LocalDate>();
	 * start.datesUntil(end).sequential().forEach(dates::add); for (int i=0;
	 * i<dates.size(); ++i) { reservedDates.add(dates.get(i).format(formatter)); } }
